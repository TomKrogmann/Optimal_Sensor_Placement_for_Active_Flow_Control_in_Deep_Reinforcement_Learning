...
import pickle
import torch as pt
from .agent import Agent, FCValue, compute_gae, compute_returns
from .attention_agent import AttentionPolicy, Attention
...
class PPO_Attention_Agent(Agent):
    def __init__(self, n_states, n_actions, action_min, action_max,
                 policy_dict: dict = DEFAULT_ATTENTION_DICT,
                 policy_epochs: int = 80,
        ...
        self._policy = AttentionPolicy(self._n_states, self._n_actions, self._action_min,
    ...
    def update(self, states: List[pt.Tensor], actions: List[pt.Tensor],
        ...
        values = [self._value(s).detach() for s in states] # for Attention: values, _ = map(list,zip(*[self._value(s).detach() for s in states]
        print("values detached shape:", len(values))
        ...
        print("log_p_old shape:", log_p_old.shape)
        ...
        #gaes = pt.cat((pt.zeros(0), gaes.repeat(log_p_old.shape[1]))).reshape(gaes.shape[0],log_p_old.shape[1])
        #print("new gaes shape:", gaes.shape)
        ...
        print("states_wf shape:", states_wf.shape)
        print("actions_wf shape:", actions_wf.shape)


        # policy update
        p_loss_, e_loss_, kl_, attention_ = [], [], [], []
        for e in range(self._policy_epochs):

            # compute loss and update weights
            attention = self._policy.compute_Attention_matrix(states_wf)
            print(attention.shape)
            log_p_new, entropy = self._policy.predict(states_wf, actions_wf)
            print("log_p_new_shape:", log_p_new.shape)
            print("entropy_shape:", entropy.shape)
            p_ratio = (log_p_new - log_p_old).exp()
            print("p_ration_shape:", p_ratio.shape)
        ...
        # value update
        v_loss_, mse_ = [], []
        # value update attention 
        #v_loss_, mse_, attention_ = [], [], []
        for e in range(self._value_epochs):
            # compute loss and update weights
            values_new = self._value(pt.cat(states))
            # Attention: values_new, attention = self._value(pt.cat(states))
            ...
            # attention_.append(attention.item())

            # check difference to old values
            with pt.no_grad():
                values_check = self._value(pt.cat(states))
                # Attention: values_new, _ = self._value(pt.cat(states))
        ...
        self._history["attention_matrix"].append(attention_)
