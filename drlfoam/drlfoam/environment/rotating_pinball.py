...
class RotatingPinball2D(Environment):
    def __init__(self, r1: float = 0.0, r2: float = 0.2):
        super(RotatingPinball2D, self).__init__(
            join(TESTCASE_PATH, "rotatingPinball2D/pinball_re100"), "Allrun.pre",
            "Allrun", "Allclean", 2, 476, 3 # MPI_ranks, n_states, n_actions - changed to 3 
        )
        self._r1 = r1
        self._r2 = r2
        self._initialized = False
        self._start_time = 0
        self._end_time = 300.125
        self._control_interval = 175
        self._train = True
        self._seed = 0
        self._action_bounds = 1.0
    ...
    def _reward(self, cx_moving_average: pt.Tensor, cy_moving_average: pt.Tensor, actions: pt.Tensor):
        
        cx_mean = pt.zeros(actions.shape[0])
        ...
        return (cx_mean.abs() - (cx_moving_average + self._r2 * cy_moving_average.abs()))
    ...
    def observations(self) -> dict:
        ...
        try:
            ...
            obs[f"rewards"] = self._reward(obs["cx_moving_average"],obs["cy_moving_average"],obs["actions"])
